    ...
    private void DrawBuildWindow()
    {
        ...
        if (GUILayout.Button("Build"))
        {
            ...
            foreach (var fileInfo in fileInfos)
            {
                ...
                string assetPaht = fileInfo.FullName.Substring(startIndex, fileInfo.FullName.Length - startIndex);
                Debug.Log(assetPaht);
                AssetImporter assetImporter = AssetImporter.GetAtPath(assetPaht);

                string abName = fileInfo.Name.Substring(0, fileInfo.Name.LastIndexOf('.'));
